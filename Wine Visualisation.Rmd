...
som_map <- ggplot(data=som_coords, aes(x=x, y=y, color=topics, size=topic_conf, alpha=topic_conf)) + 
  geom_point() +
  geom_point(data=som_medioids, aes(x=med_x, y=med_y, alpha=1), inherit.aes=FALSE) + 
  theme_classic() + 
...
```
  ...
  annotate("text", x=as.vector(som_medioids["med_x"]), y=as.vector(som_medioids["med_y"]), label="X")
```
