...
som_coords['topics'] <- as.factor(colnames(som_coords)[apply(som_coords[,1:8], 1, max_index)])
som_coords['topic_conf'] <- apply(som_coords[,1:8], 1, max)

# Convert to tibble, because tibbles and df's order factors differently!
# as_tibble(som_coords) %>% arrange(topics) -> som_coords
...
som_medioids['topics'] <- word_dist_df['topic']
...
#subset <- sum_tc[sum_tc["country"]=="Chile",]
ggplot(sum_tc, aes(x=variable, y=topic_prob, fill=variable)) + 
  scale_fill_brewer(palette="Set1") + 
  geom_col() + 
  coord_flip() + 
