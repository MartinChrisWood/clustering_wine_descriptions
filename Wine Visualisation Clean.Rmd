...
wine_df <- subset(wine_df, select = -c(X0, X4, X7, X12))
...
word_dist_df <- word_dist_df[-c(1, 5, 8, 13),]
...
wine_df['topics'] <- as.factor(apply(wine_df[,18:27], 1, max_index))

# Record for each wine its topic probability for that highest probability wine, for my own convenience later
wine_df['topic_certainty'] <- apply(wine_df[,18:27], 1, max)
```
...
wine_som <- som(as.matrix(wine_df[,18:27]), grid=som_grid, rlen=100, alpha=c(0.05, 0.01))
```
...
som_coords['topics'] <- colnames(som_coords)[apply(som_coords[,1:10], 1, max_index)]
som_coords['topic_conf'] <- apply(som_coords[,1:10], 1, max)
...
wine_df_long <- wine_df[,c("country", "description", "title", "X1", "X2", "X3", "X5", "X6", "X8", "X9", "X10", "X12", "X13")]
